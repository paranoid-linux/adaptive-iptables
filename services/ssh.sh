...
do_stop(){
    ...
    for i in ${_interfaces//,/ }; do
        iptables_whipe_chain "${i}_input_tcp_sshd"
        iptables_whipe_chain "${i}_output_tcp_sshd"
    done
}
...
do_start(){
    ...
    for i in ${_interfaces//,/ }; do
        ...
        iptables --new-chain ${i}_input_tcp_sshd
        iptables_check_before -A ${i}_input_tcp_sshd -m conntrack --ctstate ESTABLISHED -j ACCEPT
        iptables_check_before -A ${i}_input_tcp_sshd -m conntrack --ctstate NEW -m limit --limit 5/second --limit-burst 50 -j ACCEPT
        iptables_check_before -A ${i}_input_tcp_sshd -j RETURN

        iptables --new-chain ${i}_output_tcp_sshd
        iptables_check_before -A ${i}_output_tcp_sshd -m conntrack --ctstate ESTABLISHED -j ACCEPT
        iptables_check_before -A ${i}_output_tcp_sshd -j RETURN

        ## Link INPUT & OUTPUT to chains
        for p in ${__LISTEN_TCP_SERVICE_PORTS__//,/ }; do
            iptables_insert_before_logging -A INPUT -i ${i} -p tcp -m tcp -s ${_nat_ip_range} --dport ${p} -j ${i}_input_tcp_sshd
            iptables_insert_before_logging -A OUTPUT -o ${i} -p tcp -m tcp -d ${_nat_ip_range} --sport ${p} -j ${i}_output_tcp_sshd
        done
    done
}
