...
__UNBOUND_ACTIVATED_CONFIGS__="$(grep -v '#' /etc/unbound/unbound.conf /etc/unbound/unbound.conf.d/*)"
## Defaults to port 53
__LISTEN_SERVICE_PORTS__="$(awk '/port: /{print $3}' <<<"${__UNBOUND_ACTIVATED_CONFIGS__}" | head -n1)"
__LISTEN_SERVICE_PORTS__="${__LISTEN_SERVICE_PORTS__:-53}"

__DO_UDP_SERVICE_PORTS__="$(awk '/do-udp: /{print $3}' <<<"${__UNBOUND_ACTIVATED_CONFIGS__}" | head -n1)"
__DO_TCP_SERVICE_PORTS__="$(awk '/do-tcp: /{print $3}' <<<"${__UNBOUND_ACTIVATED_CONFIGS__}" | head -n1)"
...
do_stop(){
    ...
    for i in ${_interfaces//,/ }; do
        iptables_wipe_chain "${i}_input_udp_unbound"
        iptables_wipe_chain "${i}_output_udp_unbound"

        iptables_wipe_chain "${i}_input_tcp_unbound"
        iptables_wipe_chain "${i}_output_tcp_unbound"
    done
}
...
do_start(){
  ...
  for i in ${_interfaces//,/ }; do
      ...
      if [ "${__DO_UDP_SERVICE_PORTS__,,}" == 'yes' ]; then
          iptables --new-chain ${i}_input_udp_unbound
          iptables_check_before -A ${i}_input_udp_unbound -m conntrack --ctstate ESTABLISHED -j ACCEPT
          iptables_check_before -A ${i}_input_udp_unbound -m conntrack --ctstate NEW -m limit --limit 5/second --limit-burst 100 -j ACCEPT
          iptables_check_before -A ${i}_input_udp_unbound -j RETURN

          iptables --new-chain ${i}_output_udp_unbound
          iptables_check_before -A ${i}_output_udp_unbound -m conntrack --ctstate ESTABLISHED -j ACCEPT
          iptables_check_before -A ${i}_output_udp_unbound -j RETURN
      fi
      if [ "${__DO_TCP_SERVICE_PORTS__,,}" == 'yes' ]; then
          iptables --new-chain ${i}_input_tcp_unbound
          iptables_check_before -A ${i}_input_tcp_unbound -m conntrack --ctstate ESTABLISHED -j ACCEPT
          iptables_check_before -A ${i}_input_tcp_unbound -m conntrack --ctstate NEW -m limit --limit 5/second --limit-burst 100 -j ACCEPT
          iptables_check_before -A ${i}_input_tcp_unbound -j RETURN

          iptables --new-chain ${i}_output_tcp_unbound
          iptables_check_before -A ${i}_output_tcp_unbound -m conntrack --ctstate ESTABLISHED -j ACCEPT
          iptables_check_before -A ${i}_output_tcp_unbound -j RETURN
      fi

      for p in ${__LISTEN_SERVICE_PORTS__//,/ }; do
          if [ "${__DO_UDP_SERVICE_PORTS__,,}" == 'yes' ]; then
              iptables_insert_before_logging -A INPUT -i ${i} -p udp -m udp -s ${_nat_ip_range} --dport ${p} -j ${i}_input_udp_unbound
              iptables_insert_before_logging -A OUTPUT -o ${i} -p udp -m udp -d ${_nat_ip_range} --sport ${p} -j ${i}_output_udp_unbound
          fi
          if [ "${__DO_TCP_SERVICE_PORTS__,,}" == 'yes' ]; then
              iptables_insert_before_logging -A INPUT -i ${i} -p tcp -m tcp -s ${_nat_ip_range} --dport ${p} -j ${i}_input_tcp_unbound
              iptables_insert_before_logging -A OUTPUT -o ${i} -p tcp -m tcp -d ${_nat_ip_range} --sport ${p} -j ${i}_output_tcp_unbound
          fi
      done
  done
}
